    ...
    public class LocationServices: ILocationServices
    {
        ...
        public async Task<List<Store>> getStores(string qsku)
        {
            ...
            try
            {
                ...
                var resp = await new HttpClient().GetStringAsync("https://api.bestbuy.com/v1/stores((region=CA))+products?apiKey=wgd9fp6cujtdn27wm9k8rtdg&show=products.sku,products.name,products.shortDescription,products.salePrice,products.regularPrice,products.addToCartURL,products.url,products.image,products.customerReviewCount,products.customerReviewAverage,address,phone,name,lng,lat,city&format=json");
                //var resp = await new HttpClient().GetStringAsync("https://api.bestbuy.com/v1/stores((area(37,-122,900)))+products(sku%20in%20(" + qsku + "))?apiKey=wgd9fp6cujtdn27wm9k8rtdg&show=products.sku,products.name,products.shortDescription,products.salePrice,products.regularPrice,products.addToCartURL,products.url,products.image,products.customerReviewCount,products.customerReviewAverage,address,phone,name,lng,lat,city&format=js");
